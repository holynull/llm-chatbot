...
class CMCQuotesHistoricalChain(Chain):
    """
    ...
    def _call(
        ...
        try:
            res= self.seq_chain.run(question=original_question) 
    ...
    async def _acall(
        ...
        original_question=response.generations[0][0].text
        try:
            res=await self.seq_chain.arun(question=original_question) 
            return {self.output_key: res}
        except Exception as err:
            # answer=await self.answer_chain.arun(question=inputs['user_input'],context=err.args)
            return {self.output_key: err.args}
    ...
    @classmethod
    def from_llm(cls,llm:BaseLanguageModel,headers:dict,**kwargs: Any,)->CMCQuotesHistoricalChain:
        ...
        api=APIChain(
            ...
            api_docs=all_templates.cmc_quote_historical_api_doc,
