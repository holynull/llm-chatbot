...
class TaapiRSIChain(Chain):
    ...
    def from_llm(cls,llm:BaseLanguageModel,taapi_secret: str,**kwargs: Any,)->TaapiRSIChain:
        ...
        rsi_question_template=PromptTemplate(input_variables=["input"],template=taapi_templates.GENERATE_RSI_QUESTION)
        questionGenChain=LLMChain(llm=api_res_llm,prompt=rsi_question_template,**kwargs)
        headers = {
        ...
        conclusion_template=PromptTemplate(input_variables=["rsi_data"],template=taapi_templates.RSI_CONCLUSION) 
